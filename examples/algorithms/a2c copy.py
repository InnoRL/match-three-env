...
GAMMA = 0.95  # 0.99
LAMBDA = 0.95  # 0.98
LEARNING_RATE_ACTOR = 2e-3  # Default is 0.0001
LEARNING_RATE_CRITIC = 1e-3  # Default is 0.0001
LEARNING_RATE_CNN = 1e-3  # Default is 0.0001

COEFFICIENT_ENTROPY = 0.001
...
@register_pytree_node_class  # This decorator does the registration
class EnvObservationWrapper:
    def __init__(self, grid=None, time_normalized=None):
        self.grid = grid if grid is not None else jnp.zeros((9, 9), dtype=jnp.int32)
    ...
    def tree_flatten(self):
        return (self.grid, self.time_normalized), None
    ...
    def replace(self, **kwargs):
        """Needed for jax.lax.select compatibility"""
        return EnvObservationWrapper(
            grid=kwargs.get("grid", self.grid),
            time_normalized=kwargs.get("time_normalized", self.time_normalized),
        )
...
class ActorCritic(nn.Module):
    ...
    def __call__(self, x):
        print("Model apply variable x: ", x)
        x, normalized_timestep = x.grid, x.time_normalized
        encoded = encode_grid(x)
        features = self.cnn(encoded)
        # add the normalized timestep to the features
        features = jnp.concatenate([features, normalized_timestep], axis=-1)
        print("normalized_timestep shape: ", normalized_timestep.shape)
...
def train():
    ...
    key = jax.random.PRNGKey(0)
    key, subkey = jax.random.split(key)
    dummy_input = EnvObservationWrapper()
    ...
    def update_step(train_state, _) -> tuple[TrainState, dict]:
        ...
        # vmap_scale_env_step = jax.vmap(lambda obs, env_params: (obs.grid, obs.time/env_params.max_steps_in_episode), in_axes=(0, None))
        vmap_wrap_obs = jax.vmap(
            lambda obs, state: EnvObservationWrapper(
                grid=obs, time_normalized=jnp.asarray([state.time / env_params.max_steps_in_episode], dtype=jnp.float32)
            ),
            in_axes=(0, 0),
        )
        ...
        def _env_step(carry, _):
            train_state, keys, augmented_obses, states = carry
            keys = vmap_split(
                keys, 2
            ...
            logits, values = vmap_apply(train_state.params_behavior, augmented_obses)
            ...
            augmented_next_obses = vmap_wrap_obs(next_obses, next_states)
            carry = (train_state, keys, augmented_next_obses, next_states)
            data = {
                "obs_batch": augmented_obses,
        ...
        augmented_obses = vmap_wrap_obs(obses, states)
        # we dont need to split the keys here because they will be split in _env_step
        carry = (train_state, subkeys, augmented_obses, states)
        ...
        _, last_values = vmap_apply(
            train_state.params_behavior, rollouts["obs_batch"][:, -1]
        )
        ...
        def _rollout_loss_fn(
            ...
            logits, values_target = vmap_apply(params, obs)
        ...
        def _batch_loss_fn(
            ...
            losses = jax.vmap(_rollout_loss_fn, in_axes=(None, 0, 0, 0, 0, 0, 0, None))(
                params,
                obs_batch,
                actions_batch,
                old_log_probs_batch,
